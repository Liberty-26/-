# -*- coding: utf-8 -*-
"""单跑：qwen-vl-max 识别 + DeepSeek v4-flash 审核（生产配置）"""
import sys, os, time, base64, json, io, glob, asyncio
sys.path.insert(0, 'G:/SteelDigitize/backend')
from PIL import Image

from ocr import call_qwen
from calibrate import structural_decompose, calibrate_items

DATA_DIR = r"C:\Users\DIY\Pictures\数据集"
OUT = r"G:\SteelDigitize\backend\tests\model_compare"

def compress(path, max_w=1280, quality=80):
    img = Image.open(path)
    w, h = img.size
    if w > max_w:
        img = img.resize((max_w, int(h * max_w / w)), Image.LANCZOS)
    buf = io.BytesIO()
    img.save(buf, format="JPEG", quality=quality)
    return base64.b64encode(buf.getvalue()).decode()

async def main():
    imgs = [
        "微信图片_20260731134045_132_6.jpg",
        "微信图片_20260731134110_141_6.jpg",
        "微信图片_20260731134129_148_6.jpg",
    ]
    results = {}
    for name in imgs:
        path = os.path.join(DATA_DIR, name)
        b64 = compress(path)
        # 识别：qwen-vl-max + 系统真实 prompt
        t0 = time.time()
        r = await call_qwen("data:image/jpeg;base64," + b64, model="qwen-vl-max")
        t_rec = time.time() - t0
        if not r.get("success"):
            print("[%s] 识别失败: %s" % (name, r.get("error", "?")), flush=True)
            continue
        items = structural_decompose(r.get("items", []))
        # 审核：DeepSeek v4-flash（生产 calibrate_items）
        t0 = time.time()
        cal = calibrate_items(items)
        t_cal = time.time() - t0
        row = {
            "img": name, "model_rec": "qwen-vl-max", "model_cal": "deepseek-v4-flash",
            "round1": {"rows": len(items), "items": items, "sec": round(t_rec, 1)},
            "round2": {"rows": len(cal.get("items", [])), "items": cal.get("items", []), "sec": round(t_cal, 1)},
            "truncated": cal.get("truncated", False),
        }
        results[name] = row
        with open(os.path.join(OUT, "max_rec_ds_cal_%s.json" % name.replace(".jpg", "")), "w", encoding="utf-8") as f:
            json.dump(row, f, ensure_ascii=False, indent=1, default=str)
        print("[%s] 识别%.1fs+审核%.1fs R1:%d行 R2:%d行 truncated=%s" % (
            name, t_rec, t_cal, len(items), len(cal.get("items", [])), cal.get("truncated", False)), flush=True)
    with open(os.path.join(OUT, "max_ds_summary.json"), "w", encoding="utf-8") as f:
        json.dump(results, f, ensure_ascii=False, indent=1, default=str)
    print("完成", flush=True)

asyncio.run(main())
